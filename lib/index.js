...
var pushPathParameters = module.exports._pushPathParameters = function (item, validationModel, parameters) {
    ...
    _.each(item.path.restifyParams, function (param) {
        if (!_.some(validationModel, _.partial(_.has, _, param))) {
    ...
    _.each(this.server.router.mounts, function (item) {
        ...
        if (validationModel) {
            ...
            if (!_.contains(self.options.blacklist, mySwaggerPathParts)) {
                ...
                _.each(validationModel, function (scopesArray, validationScope) {
                    var realScope = internalToExternalScope[validationScope];
                    _.each(scopesArray, function (valueArray, key) {
                        ...
                        var swaggerScope = value.swaggerScope || realScope;
